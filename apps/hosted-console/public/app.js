...
function controlView() {
  ...
  queueMicrotask(renderPbxBridgeControls);
  const delivery = state.tenant.ownerDelivery;
  const callMap = state.tenant.pbxCallMap;
  const mappedEmployees = state.routeProfiles.map((profile) => `<li><strong>Ext ${escapeHtml(profile.extension)}</strong> · ${escapeHtml(profile.name)} · ${profile.forwardingConfigured ? "personal call-forward set" : "needs call-forward number"}</li>`).join("") || "<li>No employee routes yet. Add one below.</li>";
  const pbxMap = `<section class="card wide-card"><p class="eyebrow">PBX & CALL MAP</p><h2>Control where business calls ring</h2><p class="muted">This is the business-call rule: main number → extension → fallback. The current Mitel/Asterisk defaults are extension 101, 25 seconds, then voicemail. Saving changes keeps the desired map in Relay; a private PBX bridge must be connected before it changes the physical phone system.</p><form id="pbx-call-map-form" class="form-grid"><label>Phone system<select name="type"><option value="asterisk_mitel" ${callMap?.type === "asterisk_mitel" || !callMap ? "selected" : ""}>Mitel + Asterisk</option><option value="generic_sip" ${callMap?.type === "generic_sip" ? "selected" : ""}>Generic SIP / VoIP</option></select></label><label>System name<input name="label" value="${escapeHtml(callMap?.label || "Current Mitel 5330e")}" required></label><label>Ring this extension first<input name="primaryExtension" value="${escapeHtml(callMap?.primaryExtension || "101")}" inputmode="numeric" required></label><label>Ring for seconds<input name="ringSeconds" value="${escapeHtml(callMap?.ringSeconds || 25)}" inputmode="numeric" min="10" max="120" required></label><label>If unanswered<select name="fallback"><option value="voicemail" ${callMap?.fallback === "voicemail" || !callMap ? "selected" : ""}>Send to voicemail</option><option value="owner_alert" ${callMap?.fallback === "owner_alert" ? "selected" : ""}>Keep with owner for follow-up</option></select></label><label>Voicemail box<input name="voicemailExtension" value="${escapeHtml(callMap?.voicemailExtension || callMap?.primaryExtension || "101")}" inputmode="numeric"></label><button class="wide" type="submit">Save business call map</button></form><div class="split compact-split"><div><p class="eyebrow">SAVED MAP</p>${callMap ? `<p><strong>${escapeHtml(state.tenant.mainNumber)}</strong> → <strong>ext ${escapeHtml(callMap.primaryExtension)}</strong> for ${escapeHtml(callMap.ringSeconds)} seconds → <strong>${escapeHtml(callMap.fallback === "voicemail" ? `voicemail ${callMap.voicemailExtension}` : "owner follow-up")}</strong></p><span class="pill warn">${escapeHtml(callMap.bridgeState.replaceAll("_", " "))}</span>` : `<p class="muted">Save the initial map to make 101 your owner/front-desk line.</p>`}</div><div><p class="eyebrow">EMPLOYEE EXTENSIONS</p><ul class="audit">${mappedEmployees}</ul></div></div></section>`;
  ...
  return `<div class="control-grid">${pbxMap}<section class="card"><p class="eyebrow">OWNER DELIVERY</p><h2>Get every important text and voicemail</h2><p class="muted">Your owner inbox always sees the organization’s conversations. These rules also alert your personal number when a live Relay phone or PBX connection is active.</p><form id="owner-delivery-form" class="stack"><label><input name="textsToOwner" type="checkbox" ${delivery.textsToOwner ? "checked" : ""}> Alert me about every inbound text</label><label><input name="voicemailsToOwner" type="checkbox" ${delivery.voicemailsToOwner ? "checked" : ""}> Alert me about every voicemail</label><label>What goes to my personal alert number<select name="alertMode"><option value="summary" ${delivery.alertMode === "summary" ? "selected" : ""}>Private summary + secure inbox link</option><option value="full_content" ${delivery.alertMode === "full_content" ? "selected" : ""}>Full content (only after live connection)</option></select></label><p class="form-note">${delivery.alertPhoneConfigured ? "Your personal alert number is configured." : "Set your personal alert number in My profile before enabling phone alerts."}</p><button type="submit">Save owner delivery</button></form></section><section class="card"><p class="eyebrow">ADD EMPLOYEE</p><h2>One employee, one extension, one route</h2><p class="muted">This creates their route and a secure-account invite. Customers keep seeing your business number.</p><form id="employee-route-form" class="form-grid"><label>Name<input name="name" placeholder="Alex Smith" required></label><label>Email<input name="inviteEmail" type="email" placeholder="alex@company.com" required></label><label>Extension<input name="extension" inputmode="numeric" placeholder="102" required></label><label>Forward calls to<input name="callForwardNumber" inputmode="tel" placeholder="+15551234567" required></label><label>Role<select name="role"><option value="agent">Employee — assigned inbox</option><option value="manager">Manager — team inbox</option></select></label><label>AI handoff topics<input name="routingTopics" placeholder="billing, phone plans, support"></label><button class="wide" type="submit">Add employee and create invite</button></form><div id="employee-result"></div></section><section class="card wide-card"><p class="eyebrow">EMPLOYEE ROUTES</p><h2>Control extensions and forwarding</h2><p class="muted">Topics are saved for future AI/PBX routing. They do not automatically move calls or texts until a live connection is verified.</p><ul class="list">${state.routeProfiles.map(routeProfileCard).join("") || "<li class=\"empty\">Add your first employee route above.</li>"}</ul></section><section class="card"><p class="eyebrow">PHONE SYSTEM</p><h2>Plan a Mitel or SIP connection</h2><p class="muted">Add the destination here. Secure credentials and live PBX testing happen only during connection activation—never stored in this reference console.</p><form id="phone-connection-form" class="stack"><label>Phone system<select name="type"><option value="mitel">Mitel</option><option value="generic_sip">Generic SIP / VoIP</option></select></label><label>Connection name<input name="label" placeholder="Main office Mitel" required></label><label>PBX address (optional)<input name="pbxHost" placeholder="pbx.example.local"></label><label>PBX extension<input name="extension" inputmode="numeric" placeholder="101" required></label><button type="submit">Save connection plan</button></form></section><section><div class="section-head"><h2>Saved phone connections</h2><span class="muted">Not live until qualified.</span></div>${connections ? `<ul class="list">${connections}</ul>` : `<div class="card empty">No PBX or SIP connection plans yet. Relay phone pairing is in the Relay phone tab.</div>`}</section></div>`;
}

function bridgeStateClass(value) {
  return value === "ready" ? "active" : value === "revoked" ? "future" : "waiting";
}

function bridgeStateLabel(bridge) {
  if (bridge.state === "ready" && bridge.callMapState === "applied") return "Active — map reported applied";
  if (bridge.state === "ready") return "Active — map not applied";
  if (bridge.state === "revoked") return "Revoked";
  return "Waiting for heartbeat";
}

function renderPbxBridgeControls() {
  if (activeView !== "control" || !state?.permissions.manageRouting || $("#pbx-bridge-controls")) return;
  const root = $("#view .control-grid");
  if (!root) return;
  const bridges = state.pbxBridges || [];
  const options = state.phoneConnections.map((connection) => `<option value="${escapeHtml(connection.id)}">${escapeHtml(connection.label)} · ext ${escapeHtml(connection.extension)}</option>`).join("");
  const current = bridges.map((bridge) => `<li class="connection-list-item ${bridgeStateClass(bridge.state)}"><div><strong>${escapeHtml(bridge.label)}</strong><br><span class="muted">${bridge.lastHeartbeatAt ? `last heartbeat ${escapeHtml(new Date(bridge.lastHeartbeatAt).toLocaleString())}` : "not checked in yet"} · map ${escapeHtml(bridge.callMapState.replaceAll("_", " "))}</span></div><span><span class="connection-state ${bridgeStateClass(bridge.state)}">${escapeHtml(bridgeStateLabel(bridge))}</span><button class="danger compact-action" data-revoke-pbx-bridge="${escapeHtml(bridge.id)}" type="button">Revoke</button></span></li>`).join("") || "<li class=\"empty\">No private PBX Bridge is enrolled yet.</li>";
  root.insertAdjacentHTML("afterbegin", `<section id="pbx-bridge-controls" class="card wide-card"><p class="eyebrow">PRIVATE PBX BRIDGE</p><h2>Connect Relay to the phone system safely</h2><p class="muted">The Bridge runs inside your PBX network and only reports health in this first phase. It cannot change a call route from the browser. Save your call map first, then create an enrollment credential for the private Bridge installer.</p>${state.tenant.pbxCallMap ? `<form id="pbx-bridge-form" class="form-grid"><label>Bridge name<input name="label" value="Office PBX Bridge" required></label><label>Phone-system plan<select name="connectionId"><option value="">Use saved call map</option>${options}</select></label><button class="wide" type="submit">Create Bridge enrollment credential</button></form><div id="pbx-bridge-result"></div>` : `<p class="form-note">Save your business call map above before enrolling the Bridge.</p>`}<ul class="connection-list">${current}</ul></section>`);
}

function renderPbxBridgeStatus() {
  if (activeView !== "connections" || !state?.permissions.manageRouting || $("#pbx-bridge-status")) return;
  const root = $("#view .connections-page");
  if (!root) return;
  const bridges = state.pbxBridges || [];
  const active = bridges.filter((bridge) => bridge.state === "ready");
  const applied = active.filter((bridge) => bridge.callMapState === "applied");
  const cards = bridges.map((bridge) => `<li class="connection-list-item ${bridgeStateClass(bridge.state)}"><div><strong>${escapeHtml(bridge.label)}</strong><br><span class="muted">${bridge.lastHeartbeatAt ? `last heartbeat ${escapeHtml(new Date(bridge.lastHeartbeatAt).toLocaleString())}` : "waiting for its first private heartbeat"} · ${escapeHtml(bridge.callMapState.replaceAll("_", " "))}</span></div><span class="connection-state ${bridgeStateClass(bridge.state)}">${escapeHtml(bridgeStateLabel(bridge))}</span></li>`).join("") || `<li class="connection-list-item future"><div><strong>Private PBX Bridge</strong><br><span class="muted">Create an enrollment credential in Control center, then install it inside your PBX network.</span></div><span class="connection-state future">Future</span></li>`;
  root.insertAdjacentHTML("afterbegin", `<section id="pbx-bridge-status" class="card"><div class="section-head"><div><p class="eyebrow">LIVE PBX BRIDGE</p><h2>Private phone-system connection</h2></div><button class="secondary" data-view="control">Manage Bridge</button></div><div class="stats bridge-stats"><div><span class="muted">Active Bridges</span><strong class="stat-value">${active.length}</strong></div><div><span class="muted">Map reported applied</span><strong class="stat-value">${applied.length}</strong></div></div><p class="muted">A green Bridge has checked in. “Map reported applied” is only shown when the Bridge confirms that its installed phone-system route matches Relay’s desired map.</p><ul class="connection-list">${cards}</ul></section>`);
}

function connectionsView() {
  if (!state.permissions.manageRouting) return `<section class="card"><p class="eyebrow">CONNECTIONS</p><h2>Owner-controlled</h2><p class="muted">The organization owner can review business-number, phone-system, and employee-route connections.</p></section>`;
  queueMicrotask(renderPbxBridgeStatus);
  const callMap = state.tenant.pbxCallMap;
  const hasCallMap = Boolean(callMap);
  const liveGateways = state.gateways.filter((gateway) => gateway.state === "ready");
  const waitingGateways = state.gateways.filter((gateway) => gateway.state !== "ready");
  const configuredCount = Number(hasCallMap) + waitingGateways.length + state.phoneConnections.length;
  const plannedEmployees = state.routeProfiles.filter((profile) => profile.forwardingConfigured).length;
  const fallback = callMap?.fallback === "owner_alert" ? "Owner follow-up" : `Voicemail ${callMap?.voicemailExtension || callMap?.primaryExtension || "—"}`;
  const pbxLabel = callMap?.label || "PBX or VoIP phone system";
  const extensionLabel = callMap?.primaryExtension ? `Extension ${callMap.primaryExtension}` : "Choose a first extension";
  const gatewayCards = state.gateways.map((gateway) => {
    const active = gateway.state === "ready";
    return `<li class="connection-list-item ${active ? "active" : "waiting"}"><div><strong>${escapeHtml(gateway.label)}</strong><br><span class="muted">${escapeHtml(gateway.phoneNumber)} · ${gateway.lastHeartbeatAt ? `last heartbeat ${escapeHtml(new Date(gateway.lastHeartbeatAt).toLocaleString())}` : "waiting for its first heartbeat"}</span></div><span class="connection-state ${active ? "active" : "waiting"}">${active ? "Active" : "Paired — waiting"}</span></li>`;
  }).join("") || `<li class="connection-list-item future"><div><strong>Native Relay phone</strong><br><span class="muted">Pair a dedicated business phone when you want carrier SMS and mobile voicemail capture.</span></div><span class="connection-state future">Future</span></li>`;
  const pbxPlans = state.phoneConnections.map((connection) => `<li class="connection-list-item waiting"><div><strong>${escapeHtml(connection.label)}</strong><br><span class="muted">${escapeHtml(connection.type === "mitel" ? "Mitel" : "Generic SIP / VoIP")} · extension ${escapeHtml(connection.extension)}${connection.pbxHost ? ` · ${escapeHtml(connection.pbxHost)}` : ""}</span></div><span class="connection-state waiting">Planned</span></li>`).join("") || `<li class="connection-list-item future"><div><strong>PBX / SIP connection</strong><br><span class="muted">Add any Mitel or SIP phone system from Control center.</span></div><span class="connection-state future">Future</span></li>`;
  const employeeRoutes = state.routeProfiles.map((profile) => `<li class="connection-list-item ${profile.forwardingConfigured ? "waiting" : "future"}"><div><strong>Extension ${escapeHtml(profile.extension)} · ${escapeHtml(profile.name)}</strong><br><span class="muted">${profile.forwardingConfigured ? "Personal call-forward destination saved" : "Needs a call-forward destination"} · ${profile.status === "active" ? "secure inbox account active" : "account invitation pending"}</span></div><span class="connection-state ${profile.forwardingConfigured ? "waiting" : "future"}">${profile.forwardingConfigured ? "Ready to verify" : "Future"}</span></li>`).join("") || `<li class="connection-list-item future"><div><strong>Employee extensions</strong><br><span class="muted">Add employees and reserve their extensions from Control center.</span></div><span class="connection-state future">Future</span></li>`;
  return `<div class="connections-page"><div class="section-head connections-heading"><div><p class="eyebrow">YOUR RELAY MAP</p><h2>What is connected, what is waiting, and what comes next.</h2></div><button class="secondary" data-view="control">Edit connections</button></div><div class="connection-legend" aria-label="Connection status legend"><span><i class="connection-dot active"></i> Active — a current Relay heartbeat has confirmed it</span><span><i class="connection-dot waiting"></i> Configured — saved here, waiting for private activation</span><span><i class="connection-dot future"></i> Future — not set up yet</span></div><div class="stats connection-stats"><div class="card"><span class="muted">Active</span><strong class="stat-value">${liveGateways.length}</strong><span class="muted">verified Relay devices</span></div><div class="card"><span class="muted">Configured / waiting</span><strong class="stat-value">${configuredCount}</strong><span class="muted">needs live verification</span></div><div class="card"><span class="muted">Employee routes ready</span><strong class="stat-value">${plannedEmployees}</strong><span class="muted">call-forward destinations saved</span></div></div><section class="card"><p class="eyebrow">BUSINESS CALL PATH</p><h2>Desired call map</h2><p class="muted">This shows the intended route. It becomes a live phone-system path only after a private PBX bridge has been connected and qualified.</p><div class="connection-map"><article class="connection-node waiting"><span class="connection-kicker">BUSINESS NUMBER</span><strong>${escapeHtml(state.tenant.mainNumber)}</strong><small>Number plan saved in Relay</small><span class="connection-state waiting">Configured</span></article><article class="connection-node ${hasCallMap ? "waiting" : "future"}"><span class="connection-kicker">PHONE SYSTEM</span><strong>${escapeHtml(pbxLabel)}</strong><small>${hasCallMap ? "Private bridge still required" : "Choose a PBX or SIP system"}</small><span class="connection-state ${hasCallMap ? "waiting" : "future"}">${hasCallMap ? "Waiting for bridge" : "Future"}</span></article><article class="connection-node ${hasCallMap ? "waiting" : "future"}"><span class="connection-kicker">RINGS FIRST</span><strong>${escapeHtml(extensionLabel)}</strong><small>${hasCallMap ? `For ${escapeHtml(callMap.ringSeconds)} seconds` : "Set a first destination"}</small><span class="connection-state ${hasCallMap ? "waiting" : "future"}">${hasCallMap ? "Mapped in Relay" : "Future"}</span></article><article class="connection-node ${hasCallMap ? "waiting" : "future"}"><span class="connection-kicker">IF UNANSWERED</span><strong>${escapeHtml(fallback)}</strong><small>${hasCallMap ? "Saved fallback rule" : "Choose voicemail or owner follow-up"}</small><span class="connection-state ${hasCallMap ? "waiting" : "future"}">${hasCallMap ? "Mapped in Relay" : "Future"}</span></article></div></section><div class="connection-grid"><section class="card"><p class="eyebrow">ACTIVE RELAY DEVICES</p><h2>Business phones & text relay</h2><p class="muted">Only devices with a current heartbeat are marked active. A Bluetooth PBX gateway alone is not counted as a text relay.</p><ul class="connection-list">${gatewayCards}</ul><button class="secondary" data-view="phone">Pair or review Relay phones</button></section><section class="card"><p class="eyebrow">PHONE SYSTEMS</p><h2>PBX and VoIP plans</h2><p class="muted">A saved system plan is not a live PBX session. Credentials and the private bridge are set up during activation.</p><ul class="connection-list">${pbxPlans}</ul><button class="secondary" data-view="control">Add a phone system</button></section><section class="card"><p class="eyebrow">EMPLOYEE DESTINATIONS</p><h2>Extensions & call forwarding</h2><p class="muted">These routes are ready for verification, but they do not forward a physical call until the PBX bridge is live.</p><ul class="connection-list">${employeeRoutes}</ul><button class="secondary" data-view="control">Manage employees</button></section><section class="card next-steps"><p class="eyebrow">NEXT TO MAKE LIVE</p><h2>Finish in this order</h2><ol><li>Connect and qualify a private PBX bridge for your phone system.</li><li>Pair a native Relay phone if you want carrier text relay and voicemail capture.</li><li>Place a test call and text, then verify each employee’s forwarding destination.</li></ol><p class="muted">Relay will keep these stages visible so a saved plan is never presented as a live connection.</p></section></div></div>`;
}
...
function render() {
  ...
  ["control", "connections"].forEach((view) => {
    const tab = document.querySelector(`[data-view="${view}"]`);
    if (tab) tab.hidden = !state.permissions.manageRouting;
  });
  if (["control", "connections"].includes(activeView) && !state.permissions.manageRouting) activeView = "inbox";
  document.querySelectorAll(".tab").forEach((button) => button.classList.toggle("active", button.dataset.view === activeView));
  $("#view").innerHTML = activeView === "control" ? controlView() : activeView === "connections" ? connectionsView() : activeView === "team" ? teamView() : activeView === "phone" ? phoneView() : activeView === "profile" ? profileView() : inboxView();
}
...
document.addEventListener("submit", async (event) => {
  ...
  if (event.target.id === "pbx-bridge-form") { event.preventDefault(); try { const enrolled = await api("/api/pbx-bridges", { method: "POST", body: JSON.stringify(Object.fromEntries(new FormData(event.target))) }); await refresh(); const output = $("#pbx-bridge-result"); if (output) output.innerHTML = `<div class="key">Give this enrollment credential only to the private PBX Bridge installer. It is shown once and cannot be recovered.<code>${escapeHtml(enrolled.bridge.id)}:${escapeHtml(enrolled.enrollmentToken)}</code></div>`; setNotice("PBX Bridge enrollment credential created. The connection stays waiting until it checks in."); } catch (error) { setNotice(error.message); } return; }
  if (event.target.id === "pbx-call-map-form") { event.preventDefault(); try { const callMap = await api("/api/pbx-call-map", { method: "POST", body: JSON.stringify(Object.fromEntries(new FormData(event.target))) }); await refresh(); setNotice(`Business call map saved: extension ${callMap.primaryExtension} rings first, then ${callMap.fallback === "voicemail" ? `voicemail ${callMap.voicemailExtension}` : "owner follow-up"}.`); } catch (error) { setNotice(error.message); } return; }
